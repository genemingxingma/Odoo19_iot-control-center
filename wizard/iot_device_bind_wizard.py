...
class IoTDeviceBindWizard(models.TransientModel):
    ...
    serial = fields.Char(string="Switch ID/Serial")
    ...
    def _reopen_self_action(self):
        ...
        return {
            ...
            "name": _("Bind Switch by ID"),
            "res_model": "iot.device.bind.wizard",
            "view_mode": "form",
            "res_id": self.id,
            "target": "new",
        }
    ...
    def action_search_id(self):
        ...
        if not key:
            self._reset_validation(_("Please input Switch ID first."))
            return self._reopen_self_action()
        normalized_key = key.lower()
        if self.company_id not in self.env.companies:
            self._reset_validation(_("You can only bind to companies you can access."))
            return self._reopen_self_action()
        try:
            ...
            self._reset_validation(str(err))
            return self._reopen_self_action()
        switch_id = rec.module_id or rec.serial or key
        self.write(
            {
                "serial": switch_id,
                "validated": True,
                "validated_key": normalized_key,
                "candidate_switch_id": switch_id,
                "candidate_last_seen": rec.last_seen,
                "status_message": _("Found switch %s. Click Confirm Bind to continue.") % switch_id,
            }
        )
        return self._reopen_self_action()
    ...
    def _reset_validation(self, status_message=False):
        vals = {
            "validated": False,
            "validated_key": False,
        ...
        if status_message is not False:
    ...
    def action_confirm_bind(self):
        ...
        rec = self.env["iot.device"].bind_by_serial(
            rec_candidate.module_id or rec_candidate.serial or key,
        ...
        self.write(
            {
                ...
                "candidate_switch_id": False,
                "candidate_last_seen": False,
                "status_message": _("Switch %s bound successfully. You can bind next one now.") % bound_id,
            }
        )
        return self._reopen_self_action()
